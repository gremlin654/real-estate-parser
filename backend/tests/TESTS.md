...
```bash
# Создать тестовую базу данных в Docker
docker-compose exec db psql -U postgres -c "CREATE DATABASE kufar_monitor_test;"
```
...
```bash
# Тестовая БД
TEST_DATABASE_URL=postgresql+asyncpg://postgres:secret@db/kufar_monitor_test
```
  ...
  test:
    ...
    steps:
      ...
      - name: Create test database
        run: |
          psql -h localhost -U postgres -c "CREATE DATABASE kufar_monitor_test;"
      - name: Run tests
        run: |
          cd backend
...
Тесты используют PostgreSQL. Убедитесь что TEST_DATABASE_URL настроен правильно.

### Ошибка: database "kufar_monitor_test" does not exist
...
```python
# Все тесты должны быть помечены как async
@pytest.mark.asyncio
