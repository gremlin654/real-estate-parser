...
class DealFinderService:
    ...
    async def get_avg_price_per_m2(
        self, city: str, rooms: Optional[int] = None, currency: str = "usd"
    ) -> Optional[float]:
        """
        ...
        if rooms is not None and (rooms < 1 or rooms > 10):
            raise ValueError(
                f"Количество комнат должно быть от 1 до 10, получено: {rooms}"
            )
        ...
        if avg_price is None:
            logger.warning(
                f"No data for avg_price_per_m2: city={city}, rooms={rooms}, currency={currency}"
            )
            return None
    ...
    def calculate_deal_percent(
        self, current_price_per_m2: float, avg_price_per_m2: float
    ) -> float:
        """
        ...
        deal_percent = (
            (current_price_per_m2 - avg_price_per_m2) / avg_price_per_m2
        ) * 100
    ...
    async def get_deal_listings(
        ...
        avg_price_per_m2 = await self.get_avg_price_per_m2(
            city=city, rooms=rooms, currency=currency
        )
    ...
    async def get_listing_deal_metrics(
        self, listing: Listing, currency: str = "usd"
        ...
        price_per_m2 = (
            ...
            if currency == "byn"
            else float(listing.price_per_m2_usd) if listing.price_per_m2_usd else None
        )
        ...
        # Получаем среднюю цену за м²
        avg_price_per_m2 = await self.get_avg_price_per_m2(
            city=listing.city, rooms=listing.rooms, currency=currency
        )
