...
class ScanHistoryService:
    ...
    async def create_scan_record(
        self, city: str, city_name: str, trigger_type: str = "manual"
    ) -> ScanHistory:
        """Создание записи истории сканирования."""
        logger.info(f"Creating scan record for {city}, trigger: {trigger_type}")
        scan = ScanHistory(
            city=city, city_name=city_name, trigger_type=trigger_type, status="running"
        )
        self.db.add(scan)
    ...
    async def complete_scan_record(
        ...
        await self.db.commit()
        logger.info(
            f"Scan record {scan_id} completed with status: {status}, duration: {scan.duration_seconds}s"
        )
    ...
    async def get_scan_record_by_id(self, scan_id) -> Optional[ScanHistory]:
        ...
        result = await self.db.execute(
            select(ScanHistory).where(ScanHistory.id == uuid_id)
        )
        return result.scalar_one_or_none()

    async def get_scan_history(
        self, limit: int = 50, offset: int = 0, city: str = None, status: str = None
    ) -> List[ScanHistory]:
