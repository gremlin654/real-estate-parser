...
def upgrade() -> None:
    ...
    op.add_column('scan_settings', sa.Column('id_uuid', postgresql.UUID(as_uuid=True), nullable=True))
    
    # Заполнить UUID для существующих записей
    op.execute("UPDATE scan_settings SET id_uuid = gen_random_uuid() WHERE id_uuid IS NULL")
