...
class KufarHTTPScraper:
    ...
    def _extract_listings(self, soup: BeautifulSoup, base_url: str) -> List[dict]:
        ...
        listing_containers = soup.find_all(
            lambda tag: (
                tag.name in ["article", "div"]
                and (
                    tag.has_attr("data-testid")
                    and "advert" in tag.get("data-testid", "").lower()
                    or tag.has_attr("class")
                    and any(
                        "card" in str(c).lower() or "listing" in str(c).lower()
                        for c in tag.get("class", [])
                    )
                    or tag.has_attr("data-advert-id")
                )
            )
        )
        ...
        for container in listing_containers:
            try:
                ...
                location_el = container.find(
                    ...
                    and (
                        tag.has_attr("data-testid")
                        and "location" in tag.get("data-testid", "").lower()
                        or tag.has_attr("class")
                        and any(
                            "location" in str(c).lower() for c in tag.get("class", [])
                        )
                    )
                )
                if location_el:
                ...
                title_el = container.find(
                    ...
                    and (
                        tag.has_attr("data-testid")
                        and "title" in tag.get("data-testid", "").lower()
                        or tag.has_attr("class")
                ...
                if not kufar_id:
                    ...
                    kufar_id = hashlib.md5(
                        (url + str(price) + str(rooms)).encode()
                    ).hexdigest()[:16]
    ...
    async def scrape_with_pagination(
        ...
        for page_num in range(1, max_pages + 1):
            ...
            logger.info(
                f"Page {page_num}: found {len(listings)} listings, total: {len(all_listings)}"
            )
...
if __name__ == "__main__":
    ...
    for i, listing in enumerate(result[:5]):
        logger.info(
            f"{i+1}. {listing.get('title', 'N/A')[:50]} - {listing.get('price', 'N/A')} ({listing.get('address', 'N/A')})"
        )
