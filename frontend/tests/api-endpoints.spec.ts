...
test.describe('Kufar Monitor - API Endpoints', () => {
  ...
  test('должна успешно получать список городов', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/scan/cities');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать расписание сканирования', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/scan/schedule');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать текущий город', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/scan/city');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать статус сканирования', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/scan/status');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать прогресс сканирования', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/scan/progress');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать summary статистику', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/stats/summary?city=minsk');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна успешно получать список объявлений', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/listings?page=1&size=10');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна возвращать 404 для несуществующего объявления', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/listings/non-existent-id');
    expect(response.status()).toBe(404);
  });

  test('должна успешно получать историю объявления', async ({ page }) => {
    // Сначала получаем список объявлений
    const listingsResponse = await page.request.get('http://localhost:8000/api/v1/listings?page=1&size=1');
    const listings = await listingsResponse.json();
    
    if (listings.items && listings.items.length > 0) {
      const listingId = listings.items[0].id;
      const historyResponse = await page.request.get(`http://localhost:8000/api/v1/history/${listingId}`);
  ...
  test('должна обновлять расписание сканирования', async ({ page }) => {
    const response = await page.request.post('http://localhost:8000/api/v1/scan/schedule', {
      data: {
  ...
  test('должна обновлять город сканирования', async ({ page }) => {
    const response = await page.request.post('http://localhost:8000/api/v1/scan/city', {
      data: {
        city: 'mogilev',
      },
    });
  ...
  test('должна запускать ручное сканирование', async ({ page }) => {
    const response = await page.request.post('http://localhost:8000/api/v1/scan/trigger');
    expect(response.ok()).toBeTruthy();
  });

  test('должна получать price trends для статистики', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/stats/price-trends?city=minsk&rooms=1&period=12');
    expect(response.ok()).toBeTruthy();
  ...
  test('должна корректно обрабатывать неверные параметры', async ({ page }) => {
    const response = await page.request.get('http://localhost:8000/api/v1/stats/price-trends?city=invalid&rooms=999');
