...
- **Fix дедупликации price_drop (2026-05-03):**
  - Дедупликация `_is_duplicate_notification()` теперь применяется только для `new_listing`
  - Для `price_drop` отключена "вечная" дедупликация по `listing_id`, чтобы повторные снижения цены по одному объявлению отправлялись в следующих сканах
  - Файл: `app/services/telegram_notification_service.py`
- **Fix Telegram-окна выборки после ручного скана (2026-05-03):**
  - В `api/v1/scan.py` выборка для Telegram была ограничена `последними 2 минутами`, из-за чего на длинных сканах (например Минск ~13 минут) не отправлялись уведомления
  - Исправлено: выборка `new_listing` и `price_drop` теперь привязана к `start_time` текущего скана
  - Файл: `app/api/v1/scan.py`
- **UX улучшения wizard подписки Telegram (2026-05-03):**
  - Кнопка пропуска стала контекстной: вместо однотипного `Пропустить (любая цена)` теперь показываются причины по шагу (`без минимума`, `без фильтра за м²`, `снять фильтр этажей`, и т.д.)
  - Добавлен helper `_is_skip_text(...)` — корректно обрабатывает разные варианты кнопки `Пропустить` во всех шагах и в редактировании
  - В редактировании цены теперь показывается валюта текущей подписки (`USD`/`BYN`), а не хардкод `USD`
  - Исправлена опечатка в inline кнопке: `Настройки уведомлений`
  - Файлы: `app/telegram/handlers/subscriptions.py`, `app/telegram/keyboards/inline.py`
- **Защита от дублей подписок (2026-05-03):**
  - В `TelegramSubscriptionService.create_subscription(...)` добавлена проверка на 100% идентичную активную подписку до создания записи
  - Сравнение нормализует `rooms` (сортировка) и `price_per_m2_max` (Decimal с точностью до 0.01), чтобы эквивалентные фильтры считались дубликатами
  - Добавлено исключение `DuplicateSubscriptionError` и UX-обработка в Telegram handler: бот сообщает `Такая подписка уже существует` и не создаёт новую
  - Для Telegram WebApp добавлен ответ `409 Conflict` при попытке создать дубликат
  - Покрыто тестами: `tests/test_telegram_subscription_service.py`, `tests/test_telegram_handlers_subscriptions.py`
- **Telegram сообщение price_drop UX (2026-05-03):**
  - Убрана лишняя ссылка внизу сообщения (`🔗 https://...`) — оставлена только inline-кнопка `🔗 Открыть на Kufar`
  - Убрана лишняя визуальная табуляция в строке снижения цены: теперь `💵 Снижение: ...` без отступов
  - Обновлены тестовые ожидания в `tests/test_telegram_message_builder.py` и `tests/test_telegram_notification_service.py`
